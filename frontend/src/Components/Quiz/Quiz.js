    ...
    const fetchQuestions = async () => {
      try {
        ...
        const response = await axios.get(
          `${backendUrl}/getQuestions/${topicId}`,
          {
            headers: {
    ...
    <>
      {!showResult[0] && (
        <div className="quiz-container max-w-2xl mx-auto p-6 bg-white shadow-lg rounded-lg">
          <div className="timer text-xl font-bold text-right text-[#FCC822]">
            ...
            <div className="options mt-4 grid grid-cols-2 gap-4">
              {[
                ...
                <button
                  ...
                  className={`py-2 px-4 rounded hover:bg-[#FCC822] ${
                    quizData[currentQuestionIndex] &&
                    quizData[currentQuestionIndex].selectedOption === index + 1
                      ? "bg-green-500 text-white" // Show the selected option with a different style
                      : "bg-[#FCC822] text-white"
                  }`}
                >
                  {option}
                </button>
      ...
      {showResult[0] && (
        <Result
          message={showResult[1].message}
          totalScore={showResult[1].totalScore}
          questions={questions}
          quizData={quizData}
        />
      )}
