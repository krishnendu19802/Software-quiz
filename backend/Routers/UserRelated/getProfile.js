...
const getProfile = async (req, res) => {
  ...
  try {
    ...
    const [userDetails] = await db
      .promise()
      .query("SELECT name, email, admin FROM users WHERE userId = ?", [userId]);

    if (userDetails.length === 0) {
      return res.status(404).json({ error: "User not found" });
    }

    const { name, email, admin } = userDetails[0];
    ...
    const [leaderBoardData] = await db
      .promise()
      .query("SELECT score FROM leaderBoard WHERE userId = ?", [userId]);
